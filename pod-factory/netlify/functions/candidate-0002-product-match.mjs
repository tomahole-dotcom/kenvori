...
export default async(req)=>{
 try{
  ...
  for(const b of cases.slice(0,8)){
   ...
   for(const pr of providers.slice(0,4)){
    try{
     ...
     ranked.push({blueprintId:b.id,blueprint:b.title,providerId:pr.id,provider:pr.title,variantCount:enabled.length,minCostCents:costs.length?Math.min(...costs):null,variants:enabled.slice(0,30).map(x=>({id:x.id,title:x.title,cost:x.cost,isEnabled:x.is_enabled!==false,placeholders:x.placeholders||null})),shipping:s});
  ...
  return Response.json({ok:true,candidateKey:"candidate-0002",blueprintMatches:cases.length,matches:ranked.slice(0,12),selection:ranked.find(x=>Number.isFinite(x.minCostCents))||null,rateLimitSafe:true,writes:false,ordersTouched:false});
