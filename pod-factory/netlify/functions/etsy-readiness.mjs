...
export default async()=>{try{
 ...
 return Response.json({ok:shipping.ok&&processing.ok&&tax.ok,shopId:SHOP_ID,
  ...
  processing:{status:processing.status,count:processingProfiles.length,profiles:processingProfiles.map(x=>({id:x.readiness_state_id,state:x.readiness_state,min:x.min_processing_days??x.min_processing_time??x.processing_time?.min??null,max:x.max_processing_days??x.max_processing_time??x.processing_time?.max??null,raw:x}))},
