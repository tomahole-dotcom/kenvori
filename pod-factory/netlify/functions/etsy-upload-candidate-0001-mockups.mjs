...
export default async()=>{try{
 ...
 for(let i=0;i<chosen.length;i++){
   ...
   const blob=await ir.blob(); const form=new FormData(); form.append("image",blob,`candidate-0001-${chosen[i].label}.jpg`); form.append("rank",String(i+1));
 ...
 return Response.json({ok:true,listingId:LISTING,uploads:out,writes:true,stateChanged:false,publishAllowed:false,printifyPublished:false,ordersTouched:false,tokensExposed:false});
