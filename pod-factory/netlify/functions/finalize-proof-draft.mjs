// Finalizes only known proof. Printify visible is not an Etsy publish flag. Never calls publish/Etsy/orders.
...
export default async(req)=>{
 ...
 const payload={title:TITLE,description:DESCRIPTION,tags:TAGS,variants:[{id:VARIANT,price:2499,is_enabled:true}],print_areas:[{variant_ids:[VARIANT],placeholders:[{position:"front",images:[{id:IMAGE,x:.5,y:.5,scale:1,angle:0}]}]}]};
 ...
 return Response.json({ok:true,productId:d.id,title:d.title,price:d.variants?.find(v=>v.id===VARIANT)?.price,cost:d.variants?.find(v=>v.id===VARIANT)?.cost,printifyVisible:d.visible,mockups:(d.images||[]).map(x=>({src:x.src,is_default:x.is_default})),tags:d.tags||[],tagCount:(d.tags||[]).length,publishEndpointCalled:false,etsyTouched:false,ordersTouched:false});
};