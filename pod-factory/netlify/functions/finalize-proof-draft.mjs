...
export default async(req)=>{
 ...
 return Response.json({ok:true,productId:d.id,title:d.title,price:d.variants?.find(v=>v.id===VARIANT)?.price,cost:d.variants?.find(v=>v.id===VARIANT)?.cost,printifyVisible:d.visible,mockups:(d.images||[]).map(x=>({src:x.src,is_default:x.is_default})),publishEndpointCalled:false,etsyTouched:false,ordersTouched:false});
};